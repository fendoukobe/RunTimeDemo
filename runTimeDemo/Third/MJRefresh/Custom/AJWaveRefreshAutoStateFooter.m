...
- (AJWaveRefreshAnimation *)logoView {
    if (!_logoView) {
        AJWaveRefreshAnimation *logoView = [[AJWaveRefreshAnimation alloc] initWithFrame:CGRectMake(100, 100, 42, 45)
                                                                               grayImage:[UIImage imageNamed:@"222222"]
                                                                                redImage:[UIImage imageNamed:@"bga_refresh_stickiness"]];
...
- (void)placeSubviews {
    ...
    self.stateLabel.center = CGPointMake(self.mj_w * 0.5, centerY + 20);
